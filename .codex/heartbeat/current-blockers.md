...
| ALPHA-RES-005 | 2026-05-26 | Missing `Designer.cs` for migration `20260512150000_AddQuestionStatusColumns` created; stub consistent with existing project pattern. |
| ALPHA-RES-006 | 2026-05-26 | Production config template (`appsettings.Production.example.json`) created; all required keys documented with `<REPLACE>` placeholders. |
| ALPHA-RES-007 | 2026-05-26 | BE Packet E C# namespace rename complete (`Tycoon.*` → `Synaptix.*`); 987 namespace declarations and 1,491 using statements updated across 1,001 files. Non-C# services, JWT contracts, and frontend handoff doc also complete. |
| ALPHA-RES-008 | 2026-05-26 | `store_purchases_enabled` feature flag added; `EnsurePaymentsEnabledAsync` returns `403 FeatureDisabled` when flag is off; flag exposed in `GET /api/v1/app/config`; defaults `false` for Alpha. |
