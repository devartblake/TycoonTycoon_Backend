...
| 2026-06-05 | operator-investigation-player-id-normalization | `python manage.py test dashboard.tests.test_user_flow.UserInvestigationFlowTests` | pass | 10 investigation workbench tests passed; `usr_<guid>` dashboard IDs are normalized before player-scoped moderation, economy, personalization, and store calls. |
| 2026-06-05 | operator-investigation-player-id-normalization | `python manage.py check` | pass | Django system check reported no issues after the investigation route fix. |
