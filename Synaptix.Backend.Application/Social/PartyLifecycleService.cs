...
namespace Synaptix.Backend.Application.Social
{
    /// <summary>
        ...
        IPresenceReader presence)
    {
        public async Task ClosePartiesForMatchAsync(Guid matchId, string reason, CancellationToken ct)
        {
            ...
            foreach (var pid in partyIds)
            {
                ...
                var roster = new Synaptix.Shared.Contracts.Dtos.PartyRosterDto(
                    PartyId: party.Id,
                    LeaderPlayerId: party.LeaderPlayerId,
                    Status: "Closed",
                    Members: members.Where(m => m.PartyId == pid)
                        .OrderBy(m => m.JoinedAtUtc)
                        .Select(m => new Synaptix.Shared.Contracts.Dtos.PartyMemberDto(
                            PlayerId: m.PlayerId,
