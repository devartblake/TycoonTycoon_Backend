...
_Last updated: 2026-06-13 — **Alpha audit remediation** (PRs #389/#390 backend, #269/#270 client): all 6 Before-Alpha blockers ✅ (§2.1 secrets, §2.2 duplicate health route, §1.1 device-first auth, §1.2 `/api/v1`, §1.3 KMS handshake proxy, §3.3 https-downgrade guard); During-Alpha §1.6 server-authoritative quiz rewards ✅, §2.3 Redis presence (required in staging/prod) ✅, §2.4 `UseForwardedHeaders` + re-keyed rate limiters ✅, §2.5 migration deploy-gating ✅, §3.4 route-parity contract test ✅, §3.2 mobile timeouts ✅, backend tests → `/api/v1` ✅, client provider-button gating ✅; EF snapshot desync repaired ✅; backend test suite 33 failures → 0 ✅. **Still open:** §1.1 real Game Center/Play Games/OAuth verification (needs provider credentials), §3.1 off-bundle assets, §1.4/§1.5/§1.7/§2.6–2.8 Before-Beta hygiene. See `Synaptix_Audit_Report.md` Part 5._
...
| Operator Dashboard Wave D (Django cutover + Blazor decommission) | Low | Repo-code parity, migration/seed bootstrap, first operational drilldown slice, CI/readiness automation, evidence package, and repo verification baseline complete; external gates remain: staging parallel-run, EF migration apply/readiness, operator sign-off, cutover, Blazor decommission after rollback window | Depends on sign-off |
...
## Operator Dashboard Detail Maturity

- [x] Operational investigation workbench for account, activity, moderation, economy, personalization, and store-stock context (`/users/{userId}/investigation`).
- [ ] Near-realtime freshness for high-value health, queue, and incident surfaces after cutover.
- [ ] Expanded analytics/reporting snapshots after operational drilldowns are validated by operators.
