...
- [ ] **Issue: Decide and document authoritative Operator Dashboard container target (Blazor vs dashboard-web).**
  ...
  - **Acceptance Criteria:**
    - [ ] Decision recorded in README/docs.
    - [ ] Any non-authoritative Dockerfiles are archived or marked experimental.
    - [ ] CI/build docs reference only the authoritative target.

## 2) Votes Migration Changelog Drift (`SEQ-2`)
- [ ] **Issue: Close stale changelog item that says votes migration is pending.**
  - **Context:** `votes` schema already exists in migration `20260319000000_AddGameEventTables`.
  - **Acceptance Criteria:**
    - [ ] Changelog pending section updated to reflect that vote schema migration is already landed.
