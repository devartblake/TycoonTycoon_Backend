...
public sealed class EfModelKeyTests
{
    ...
    {
        var opts = new DbContextOptionsBuilder<Synaptix.Backend.Infrastructure.Persistence.AppDb>()
            .UseInMemoryDatabase("model_key_test") // model build only
            .Options;

        using var db = new Synaptix.Backend.Infrastructure.Persistence.AppDb(opts);
