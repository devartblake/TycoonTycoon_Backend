 ...
 */
import { generateMockUsers, generateMockUsersList, MOCK_SAVED_VIEWS, MOCK_TEMPLATES, MOCK_CHANNELS, MOCK_SCHEDULES, MOCK_DEAD_LETTERS, generateMockAntiCheatFlag, MOCK_ANTI_CHEAT_STATS, } from './mock-data';
...
// ============ Users Mock API ============
export async function mockGetUsers(_filters) {
    await delay();
    return generateMockUsersList();
}
export async function mockGetUserDetail(_userId) {
    await delay();
    const users = generateMockUsers(1);
    return { ...users[0], id: _userId };
}
export async function mockBanUser(_userId, _reason) {
    await delay();
    return { success: true };
}
export async function mockUnbanUser(_userId) {
    await delay();
    return { success: true };
}
export async function mockGetSavedViews() {
    await delay();
    return MOCK_SAVED_VIEWS;
}
export async function mockCreateSavedView(name, filters) {
    await delay();
    return {
        id: `view_${Date.now()}`,
        name,
...
export async function mockDeleteSavedView(_viewId) {
    await delay();
    return { success: true };
}
...
export async function mockCreateTemplate(payload) {
    await delay();
    return {
        id: `tpl_${Date.now()}`,
        ...payload,
...
export async function mockUpdateTemplate(templateId, payload) {
    await delay();
...
export async function mockDeleteTemplate(_templateId) {
    await delay();
    return { success: true };
}
export async function mockGetChannels() {
    await delay();
    return MOCK_CHANNELS;
}
export async function mockUpdateChannel(channelId, enabled, config) {
    await delay();
...
export async function mockGetSchedules() {
    await delay();
    return MOCK_SCHEDULES;
}
export async function mockCreateSchedule(payload) {
    await delay();
    return {
        id: `sch_${Date.now()}`,
        ...payload,
...
export async function mockCancelSchedule(_scheduleId) {
    await delay();
    return { success: true };
}
export async function mockGetDeadLetterMessages() {
    await delay();
    return MOCK_DEAD_LETTERS;
}
export async function mockRetryDeadLetter(_messageId) {
    await delay();
    return { success: true };
}
// ============ Anti-Cheat Mock API ============
export async function mockGetQueueStats() {
    await delay();
    return MOCK_ANTI_CHEAT_STATS;
}
export async function mockGetNextFlag() {
    await delay();
    return generateMockAntiCheatFlag();
}
export async function mockGetFlagDetail(flagId) {
    await delay();
    return generateMockAntiCheatFlag(flagId);
}
export async function mockSubmitVerdict(_payload) {
    await delay();
...
// ============ Audit Mock API ============
export async function mockGetAuditEvents(_filters, offset = 0, limit = 50) {
    await delay();
    const events = [];
...
export async function mockGetAuditStats() {
    await delay();
    return {
        totalEvents: 2847,
        successRate: 0.96,
...
export async function mockGetIPLocations(_filters) {
    await delay();
    const countries = ['US', 'UK', 'CA', 'DE', 'JP', 'AU', 'FR', 'SG'];
    const cities = {
        US: [
...
export async function mockGetEventDetail(eventId) {
    await delay();
    return {
        id: eventId,
        eventType: 'api_call',
...
export async function mockGetServiceHistory(serviceId, hours = 24) {
    await delay();
    const metrics = [];
    const now = Date.now();
...
export async function mockGetAllServiceHistory(hours = 24) {
    await delay();
    const serviceIds = ['api-gateway', 'auth-service', 'user-service', 'database', 'cache', 'storage'];
    const histories = await Promise.all(serviceIds.map((id) => mockGetServiceHistory(id, hours)));
    return histories;
}
// ============ Moderation Mock API ============
export async function mockGetPlayerModeration(playerId) {
    await delay();
...
export async function mockBanPlayer(playerId, reason, notes) {
    await delay();
    return {
        id: `action_${Date.now()}`,
        playerId,
...
export async function mockUnbanPlayer(playerId, reason) {
    await delay();
    return {
        id: `action_${Date.now()}`,
        playerId,
...
export async function mockSuspendPlayer(playerId, durationHours, reason, notes) {
    await delay();
...
export async function mockUnsuspendPlayer(playerId, reason) {
    await delay();
    return {
        id: `action_${Date.now()}`,
        playerId,
...
export async function mockWarnPlayer(playerId, reason, notes) {
    await delay();
    return {
        id: `action_${Date.now()}`,
        playerId,
...
export async function mockAddModeratorNote(playerId, note) {
    await delay();
    return {
        id: `action_${Date.now()}`,
        playerId,
...
// ============ Economy Mock API ============
export async function mockGetPlayerEconomy(playerId) {
    await delay();
...
export async function mockGetPlayerTransactions(playerId, _filters, offset = 0, limit = 50) {
    await delay();
    const types = ['purchase', 'earn', 'refund', 'adjustment', 'reward', 'penalty'];
    const transactions = [];
    let balance = 50000;
    for (let i = 0; i < limit; i++) {
...
export async function mockAdjustBalance(_adjustment) {
    await delay();
...
export async function mockIssueRefund(_playerId, _transactionId, _reason, _note) {
    await delay();
    return {
        success: true,
...
export async function mockGetEconomyStats() {
    await delay();
    return {
        totalPlayers: 5247,
...
export async function mockSearchPlayers(query, limit = 20) {
    await delay();
    const results = [];
...
export async function mockGetQuestions(_filters, offset = 0, limit = 50) {
    await delay();
...
export async function mockReviewQuestion(_review) {
    await delay();
    return {
        success: true,
...
export async function mockBulkReviewQuestions(questionIds, _verdict, _reason) {
    await delay();
    return {
        success: true,
...
export async function mockGetQuestionsStats() {
    await delay();
    return {
        totalPending: 247,
        totalApproved: 5234,
...
// ============ Operations Mock API ============
export async function mockGetSeasons(_filters, offset = 0, limit = 50) {
    await delay();
...
export async function mockGetGameEvents(_filters, offset = 0, limit = 50) {
    await delay();
    const statuses = ['draft', 'upcoming', 'active', 'ended', 'cancelled'];
    const types = ['tournament', 'challenge', 'promotion', 'special'];
    const events = [];
    for (let i = 0; i < limit; i++) {
        const status = statuses[Math.floor(Math.random() * statuses.length)];
        const type = types[Math.floor(Math.random() * types.length)];
...
export async function mockPerformLifecycleAction(_action) {
    await delay();
    return {
        success: true,
...
export async function mockGetOperationsStats() {
    await delay();
    return {
        activeSeasons: 2,
        upcomingEvents: 8,
...
export async function mockGetSeason(seasonId) {
    await delay();
    return {
        id: seasonId,
        name: 'Current Season',
...
export async function mockGetEvent(eventId) {
    await delay();
    return {
        id: eventId,
...
export async function mockGetProducts(_offset = 0, limit = 50) {
    await delay();
    const products = [];
    for (let i = 0; i < limit; i++) {
        products.push({
...
export async function mockUpdateProduct(_id, product) {
    await delay();
    return { id: _id, ...product, updatedAt: new Date().toISOString() };
}
export async function mockDeleteProduct(_id) {
    await delay();
    return { success: true };
}
export async function mockGetFlashSales(_offset = 0, limit = 50) {
    await delay();
    const sales = [];
    for (let i = 0; i < limit; i++) {
...
export async function mockCreateFlashSale(sale) {
    await delay();
    return { id: `sale_${Date.now()}`, ...sale, createdAt: new Date().toISOString() };
}
export async function mockUpdateFlashSale(_id, sale) {
    await delay();
    return { id: _id, ...sale };
}
export async function mockDeleteFlashSale(_id) {
    await delay();
    return { success: true };
}
export async function mockGetStockPolicies(_offset = 0, limit = 50) {
    await delay();
    const policies = [];
    for (let i = 0; i < limit; i++) {
        policies.push({
...
export async function mockCreateStockPolicy(policy) {
    await delay();
    return { id: `policy_${Date.now()}`, ...policy, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
}
export async function mockUpdateStockPolicy(_id, policy) {
    await delay();
    return { id: _id, ...policy, updatedAt: new Date().toISOString() };
}
export async function mockDeleteStockPolicy(_id) {
    await delay();
    return { success: true };
}
export async function mockGetRewardLimits(_offset = 0, limit = 50) {
    await delay();
    const limits = [];
    for (let i = 0; i < limit; i++) {
...
export async function mockCreateRewardLimit(limit) {
    await delay();
    return { id: `limit_${Date.now()}`, ...limit, createdAt: new Date().toISOString() };
}
export async function mockUpdateRewardLimit(_id, limit) {
    await delay();
    return { id: _id, ...limit };
}
export async function mockDeleteRewardLimit(_id) {
    await delay();
    return { success: true };
}
export async function mockGetStoreStats() {
    await delay();
    return {
        totalProducts: 847,
        activeFlashSales: 12,
