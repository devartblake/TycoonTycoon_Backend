...
function mapSeasonStatus(status) {
...
function mapEventType(kind) {
    const k = kind.toLowerCase();
    if (k.includes('tournament'))
        return 'tournament';
...
export async function getSeasons(filters, offset = 0, limit = 50) {
    ...
    const res = await apiGet(`/admin/seasons?${params}`);
...
export async function getSeason(seasonId) {
    ...
    const res = await apiGet('/admin/seasons?page=1&pageSize=200');
...
export async function getGameEvents(filters, offset = 0, limit = 50) {
    ...
    const res = await apiGet(`/admin/game-events/?${params}`);
...
export async function getEvent(eventId) {
    ...
    const res = await apiGet('/admin/game-events/?page=1&pageSize=200');
    const dto = res.items.find((e) => e.id === eventId);
    if (!dto)
        throw new Error(`Game event ${eventId} not found`);
...
export async function getOperationsStats() {
    ...
    const [seasonsRes, eventsRes] = await Promise.all([
        apiGet('/admin/seasons?page=1&pageSize=200'),
        apiGet('/admin/game-events/?page=1&pageSize=200'),
    ]);
