...
export async function getPlayerEconomy(_playerId) {
    if (getMockMode())
        return mockApi.mockGetPlayerEconomy(_playerId);
...
export async function getPlayerTransactions(playerId, filters, offset = 0, limit = 50) {
    if (getMockMode())
        return mockApi.mockGetPlayerTransactions(playerId, filters, offset, limit);
    const res = await apiGet(`/admin/economy/history/${playerId}?page=${offsetToPage(offset, limit)}&pageSize=${limit}`);
...
export async function adjustBalance(adjustment) {
    if (getMockMode())
        return mockApi.mockAdjustBalance(adjustment);
    // Post a single-line Coins ledger transaction (idempotent by EventId).
    const res = await apiPost('/admin/economy/transactions', {
...
export async function issueRefund(playerId, transactionId, reason, _note) {
    if (getMockMode())
        return mockApi.mockIssueRefund(playerId, transactionId, reason, _note);
...
export async function getEconomyStats() {
    if (getMockMode())
        return mockApi.mockGetEconomyStats();
...
export async function searchPlayers(query, limit = 20) {
    if (getMockMode())
        return mockApi.mockSearchPlayers(query, limit);
    // Backend exposes a single-match resolver, not a search. Wrap the match (if any).
    try {
        const res = await apiGet(`/admin/player-lookup/resolve?query=${encodeURIComponent(query)}`);
