...
export async function getServiceHistory(serviceId, _hours = 24) {
    if (getMockMode())
        return mockApi.mockGetServiceHistory(serviceId, _hours);
    // No backend time-series source (see #418).
    void _hours;
    return { serviceId, metrics: [] };
}
export async function getAllServiceHistory(_hours = 24) {
    if (getMockMode())
        return mockApi.mockGetAllServiceHistory(_hours);
    // No backend time-series source (see #418).
    void _hours;
    return [];
}
