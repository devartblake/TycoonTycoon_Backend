...
export function useStartInstallation() {
    ...
    return useMutation({
        mutationFn: (config) => api.startInstallation(config),
...
export function useUploadBundle() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ file, onProgress }) => api.uploadBundle(file, onProgress),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['available-bundles'] }),
    });
}
export function useValidateBundle() {
    return useMutation({
        mutationFn: (bundleId) => api.validateBundle(bundleId),
    });
}
export function useDeployBundle() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (bundleId) => api.deployBundle(bundleId),
        onSuccess: () => {
...
export function useDeleteBundle() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (bundleId) => api.deleteBundle(bundleId),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['available-bundles'] }),
    });
}
// Configuration
...
export function useUpdateInstallerConfig() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (config) => api.updateInstallerConfig(config),
...
// Logs
export function useInstallationLogs(stepId) {
    return useQuery({
