...
function toAuditEvent(dto) {
    return {
        ...
        details: dto.metadata ?? undefined,
    };
}
export async function getAuditEvents(filters, offset = 0, limit = 50) {
    ...
    const res = await apiGet(`/admin/audit/security?${params}`);
    ...
    if (filters?.adminEmail)
        items = items.filter((e) => e.adminEmail.includes(filters.adminEmail));
...
export async function getAuditStats() {
    ...
    const res = await apiGet('/admin/audit/security?page=1&pageSize=200');
...
export async function getIPLocations(_filters) {
    if (getMockMode())
        return mockApi.mockGetIPLocations(_filters);
...
export async function getEventDetail(eventId) {
    ...
    const dto = await apiGet(`/admin/audit/security/${eventId}`);
