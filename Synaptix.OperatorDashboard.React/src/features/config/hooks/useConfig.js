...
export function useToggleFeatureFlag() {
    ...
    return useMutation({
        mutationFn: ({ id, enabled }) => api.toggleFeatureFlag(id, enabled),
...
export function useUpdateFeatureFlag() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, ...data }) => api.updateFeatureFlag(id, data),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['feature-flags'] }),
    });
}
export function useDeleteFeatureFlag() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (id) => api.deleteFeatureFlag(id),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['feature-flags'] }),
    });
}
// Admin ACL
...
export function useUpdateAdminACL() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, ...data }) => api.updateAdminACL(id, data),
...
export function useDeleteAdminACL() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (id) => api.deleteAdminACL(id),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin-acl'] }),
    });
}
// System Config
...
export function useUpdateSystemConfig() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (config) => api.updateSystemConfig(config),
...
export function useSetMaintenanceMode() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ enabled, message }) => api.setMaintenanceMode(enabled, message),
        onSuccess: () => {
