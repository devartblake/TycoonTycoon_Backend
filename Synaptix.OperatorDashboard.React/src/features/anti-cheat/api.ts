...
interface BackendFlagListResponse {
  page: number
  pageSize: number
  total: number
  items: BackendFlagDto[]
}

function mapSeverity(severity: number): AntiCheatFlag['flagSeverity'] {
  switch (severity) {
    case 0:
      return 'low'
    case 1:
...
function toFlag(dto: BackendFlagDto): AntiCheatFlag {
  return {
    id: dto.id,
...
export async function getQueueStats(): Promise<QueueStats> {
  ...
  const res = await apiGet<BackendFlagListResponse>('/admin/anti-cheat/flags?unreviewedOnly=true&page=1&pageSize=1')
...
export async function getNextFlag(): Promise<AntiCheatFlag> {
  ...
  const res = await apiGet<BackendFlagListResponse>('/admin/anti-cheat/flags?unreviewedOnly=true&page=1&pageSize=1')
  if (res.items.length === 0) throw new Error('No pending anti-cheat flags.')
...
export async function getFlagDetail(flagId: string): Promise<AntiCheatFlag> {
  ...
  const res = await apiGet<BackendFlagListResponse>('/admin/anti-cheat/flags?page=1&pageSize=100')
  const dto = res.items.find((f) => f.id === flagId)
  if (!dto) throw new Error(`Anti-cheat flag ${flagId} not found`)
  return toFlag(dto)
}
