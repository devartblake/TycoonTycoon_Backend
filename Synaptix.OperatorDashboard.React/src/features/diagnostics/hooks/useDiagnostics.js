...
export function useProbeHistory(probeId) {
    return useQuery({
...
export function useProbeLogs(probeId) {
    return useQuery({
...
export function useRunProbeNow() {
    ...
    return useMutation({
        mutationFn: (probeId) => api.runProbeNow(probeId),
        onSuccess: () => {
...
export function useUpdateProbe() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ probeId, data }) => api.updateProbe(probeId, data),
...
export function useEnableProbe() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (probeId) => api.enableProbe(probeId),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['probe-records'] }),
    });
}
export function useDisableProbe() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (probeId) => api.disableProbe(probeId),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['probe-records'] }),
    });
}
